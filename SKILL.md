...
After `make install`, source the Cloudberry environment script so shell tools (`gpstart`, `gpinitsystem`, etc.) are on your PATH:

```bash
source $INSTALL_PREFIX/cloudberry-env.sh
```

This sets `GPHOME`, `PATH`, `LD_LIBRARY_PATH`, and `PYTHONPATH`. This is required **before** creating the demo cluster.

**Note**: `gpAux/gpdemo/gpdemo-env.sh` does **not** exist yet at this point — it is generated by `make create-demo-cluster` in the next step.
...
**Prerequisite**: You must have sourced `cloudberry-env.sh` (Step 4) before this step.
...
After creation, `gpAux/gpdemo/gpdemo-env.sh` is generated. Source it to set `COORDINATOR_DATA_DIRECTORY` and `PGPORT`:

```bash
source gpAux/gpdemo/gpdemo-env.sh
```

### Save environment for daily use

Now that both environment files exist, create a dedicated env file for future sessions. **Do NOT** add these to `~/.bashrc` — database environment variables should not pollute the default shell, especially when the system may run multiple database instances.

```bash
cat > ~/.cloudberry.env <<EOF
source $INSTALL_PREFIX/cloudberry-env.sh
source $(pwd)/gpAux/gpdemo/gpdemo-env.sh
EOF
```

In subsequent sessions, source this single file before working with Cloudberry:
...
```bash
# Stop cluster if running (gpdemo-env.sh may not exist on a fresh build)
source $INSTALL_PREFIX/cloudberry-env.sh
[ -f gpAux/gpdemo/gpdemo-env.sh ] && source gpAux/gpdemo/gpdemo-env.sh
...
# Set up environment (cloudberry-env.sh needed before creating cluster)
source $INSTALL_PREFIX/cloudberry-env.sh

# Create cluster (this generates gpdemo-env.sh)
make create-demo-cluster

# Now gpdemo-env.sh exists, source it
source gpAux/gpdemo/gpdemo-env.sh
gpstart -a
gpstate
```
...
| `$INSTALL_PREFIX/cloudberry-env.sh` | Environment setup (exists after `make install`) |
| `gpAux/gpdemo/gpdemo-env.sh` | Demo cluster environment (generated by `make create-demo-cluster`) |
